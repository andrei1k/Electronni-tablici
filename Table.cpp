...
void Table::load(const char* path)
{
...
void Table::save(const char* path) const
{
...
void Table::print() const
{
	for (unsigned i = 0, column = 1, row = 1; i < this->valuesOfTable.size(); i++) {
		...
		//std::cout << this->valuesOfTable[i]->getPosition().fst << ' ' << this->valuesOfTable[i]->getPosition().snd << ' ';
		if(this->valuesOfTable[i]->getType() == STRING)
...
{
	if (typeOfString(newValue) == INVALID || column > this->numOfColumns || column < 1 || row > (this->valuesOfTable.size()/this->numOfColumns) || row < 1) {
...
std::string Table::needsCommaAfter(std::string buffer)
{
	for (unsigned i = 0; i < buffer.size(); i++) {
		...
		if (buffer[i] == ' ') {
			...
			//std::cout << buf1 << typeOfString(buf1) << "," << buf2 << typeOfString(buf2) << '\n';
			if (typeOfString(buf1) != INVALID && typeOfString(buf2) != INVALID) {
				return buf1;
			}
		}
	}
...
//Table* Table::extraction(std::string whatToExtract) const {}
...
{
	if (one->getType() == two->getType() && (one->getType() == STRING || one->getType() == EMPTY)) {
		...
		if (one->getType() == EMPTY)
			return 0;
		if (two->getType() == EMPTY)
			return 1;
